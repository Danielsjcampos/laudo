...
import { mockClinics, mockDoctors, mockExams, mockGlobalStats } from '../../../data/mockData';
        ...
        <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm text-center md:text-left">Visão geral do ecossistema LaudoDigital</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                <Card title="Clínicas na Rede" value={mockGlobalStats.activeClinics} icon={<ClinicIcon className="h-6 w-6 md:h-8 md:w-8" />} color="blue" />
                <Card title="Corpo Clínico" value={mockGlobalStats.totalDoctors} icon={<UsersIcon className="h-6 w-6 md:h-8 md:w-8" />} color="green" />
                <Card title="Volume de Laudos" value={mockGlobalStats.totalExamsProcessed.toLocaleString('pt-BR')} icon={<FileTextIcon className="h-6 w-6 md:h-8 md:w-8" />} color="yellow" />
                <Card title="Faturamento Bruto" value={`R$ ${mockGlobalStats.totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon={<WalletIcon className="h-6 w-6 md:h-8 md:w-8" />} color="blue" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div className="bg-gradient-to-br from-brand-blue-900 to-brand-blue-700 p-6 md:p-8 rounded-3xl shadow-xl text-white flex flex-col justify-between">
                    <div>
                        ...
                        <p className="text-3xl md:text-4xl font-extrabold mb-1">R$ {mockGlobalStats.platformProfit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                    ...
                    <div className="mt-8 space-y-4">
                        <div className="flex justify-between text-xs">
                            ...
                            <span className="font-bold">R$ {mockGlobalStats.totalTransferred.toLocaleString('pt-BR')}</span>
                        </div>
