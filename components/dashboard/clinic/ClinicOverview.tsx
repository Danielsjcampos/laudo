...
const ClinicOverview: React.FC<ClinicOverviewProps> = ({ exams, patients, onRequestExam, onNavigateToExams }) => {
    const [isRequestModalOpen, setIsRequestModalOpen] = useState(false);

    const pendingExams = exams.filter(e => e.status !== 'Concluído').length;
    const completedExams = exams.filter(e => e.status === 'Concluído').length;
    const totalRevenue = mockClinicPerformance.weeklyExams.reduce((acc, curr) => acc + curr.profit, 0);
        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div className="space-y-1">
                    ...
                    <p className="text-gray-500 font-medium">Controle de fluxo, laudos e performance clínica em tempo real.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col justify-between">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Exames Hoje</p>
                ...
                <div className="bg-brand-teal-500 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        ...
                        <div>
                            <p className="text-4xl font-black">1.4 <span className="text-sm opacity-70">DIAS</span></p>
                ...
                <div className="bg-gray-900 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        <p className="text-[10px] font-black uppercase tracking-widest text-brand-blue-400">Lucro Semanal</p>
                        <p className="text-3xl font-black">R$ {totalRevenue.toLocaleString()}</p>
                        <div className="mt-2 text-[10px] font-black text-green-400 uppercase tracking-tighter">↑ 12% vs sem. ant</div>
                    </div>
                </div>
            </div>

            {/* Grid de Gráficos de Fluxo */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm flex flex-col min-h-[450px]">
                    ...
                    <div className="flex-1 flex items-end justify-between space-x-4">
                        {mockClinicPerformance.weeklyExams.map((data, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center group">
                                <div className="relative w-full flex flex-col items-center justify-end h-64">
                                    ...
                                    <div 
                                        ...
                                        style={{ height: `${(data.count / 110) * 100}%` }}
                                    >
                                ...
                                <span className="mt-4 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{data.day}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="space-y-8 flex flex-col">
                    <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm flex-1">
                        ...
                        <div className="mt-10 p-6 bg-brand-blue-50 rounded-[2rem] border border-brand-blue-100">
                             ...
                             <p className="text-xs text-brand-blue-900 font-medium italic">Aumente a oferta de exames de Cardiologia no marketplace para otimizar custos.</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-brand-blue-600 to-brand-blue-800 p-10 rounded-[3rem] text-white shadow-xl relative overflow-hidden group">
                        <h3 className="text-[10px] font-black uppercase tracking-widest opacity-70 mb-4">Ganhos de Eficiência</h3>
                        <div className="flex items-baseline space-x-2">
                            <span className="text-4xl font-black">R$ 18.2k</span>
                            <span className="text-brand-blue-300 font-bold text-xs">ECONOMIZADO</span>
                        </div>
                        <p className="text-xs text-brand-blue-200 mt-2 font-medium">Uso de IA na pré-análise de laudos</p>
            ...
            <div className="bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden">
                 <div className="p-10 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-50">
                    <h2 className="text-2xl font-black text-gray-800 tracking-tight">Rastreamento Ativo</h2>
                    <div className="flex space-x-2">
                        ...
                        <button className="bg-brand-blue-50 text-[10px] font-black uppercase px-4 py-2 rounded-xl text-brand-blue-600">Pendentes</button>
                    </div>
                 </div>
                                    ...
                                    <td className="px-10 py-6">
                                        <div className="flex items-center">
                                            <div className="w-8 h-8 rounded-full bg-brand-blue-50 text-brand-blue-600 flex items-center justify-center font-black text-[10px] mr-3 shadow-sm">
                                                {exam.doctorAssignedName?.charAt(4) || '??'}
                                            </div>
                                            <div>
                                                <span className="text-sm font-bold text-gray-800">{exam.doctorAssignedName || 'Aguardando...'}</span>
                                                {exam.doctorAssignedName && <p className="text-[9px] text-brand-teal-500 font-bold uppercase tracking-tighter">Médico Especialista</p>}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-10 py-6">
