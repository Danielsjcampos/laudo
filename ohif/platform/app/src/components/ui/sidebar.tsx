...
export const SidebarBody = (props: any) => {
  return (
    <>
      ...
      {/* MobileSidebar disabled to prevent 'fixed' overlay bug causing overlap in OHIF */}
    </>
  );
};
...
export const DesktopSidebar = ({ className, children, ...props }: any) => {
  ...
  return (
    <motion.div
      className={cn(
        'flex h-full flex-shrink-0 flex-col border-r border-neutral-800 bg-[#0a0a0a] px-4 py-4 transition-all duration-300',
        className
      )}
      animate={{
        width: animate ? (open ? '200px' : '60px') : '200px', // Narrower expanded width
      }}
