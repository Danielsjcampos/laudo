    ...
    return (
        <div className="bg-black rounded-3xl overflow-hidden shadow-sm border border-gray-800 flex flex-col h-[600px] relative group">
             ...
             <div className="absolute top-4 left-4 text-[10px] font-mono text-cyan-500 z-10 pointer-events-none">
                 <p>ID: {dicomUrl?.split('-').pop()?.slice(0, 8) || '849302'}</p>
                 <p>NAME: {examImageUrl ? 'DATA EXTRACTED' : 'ANONYMIZED'}</p>
                 <p>MOD: {modality || 'OT'}</p>
             ...
             <div className="flex-1 bg-black flex items-center justify-center relative overflow-hidden">
                ...
                {examImageUrl ? (
                    <div className="w-full h-full flex items-center justify-center bg-gray-950">
                        <img 
                            src={examImageUrl.startsWith('http') ? examImageUrl : `${API_BASE}${examImageUrl}`}
                            alt="Visualização Médica"
