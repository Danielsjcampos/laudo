    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Requisitar Novo Laudo">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div 
          ...
          className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center transition-colors cursor-pointer group ${file ? 'border-brand-teal-500 bg-brand-teal-50' : 'border-gray-300 bg-gray-50 hover:bg-brand-blue-50 hover:border-brand-blue-300'}`}
        >
            <input 
              id="dicom-upload"
              type="file"
            ...
            />
            <div className={`p-3 rounded-full shadow-sm group-hover:shadow-md mb-3 ${file ? 'bg-brand-teal-500 text-white' : 'bg-white text-gray-400 group-hover:text-brand-blue-600'}`}>
                 {file ? (
            ...
            <p className="text-sm font-medium text-gray-700 max-w-full truncate px-4">
                {file ? (file.name.length > 40 ? file.name.substring(0, 40) + '...' : file.name) : 'Clique para selecionar arquivo DICOM'}
            </p>
            <p className="text-xs text-gray-500 mt-1">Suporta .dcm (Max 500MB)</p>
        </div>
